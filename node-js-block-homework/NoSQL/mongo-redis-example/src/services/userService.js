...
const findUserById = async (userId) => {
    ...
    if (user) {
        await client.set(cacheKey, JSON.stringify(user), {EX: 100});
    }
    return user;
};
...
const createUser = async (data) => {
    ...
    await client.set(cacheKey, JSON.stringify(newUser), {EX: 300});
    return newUser;
};
...
const updateUser = async (data) => {
    ...
    if (updatedUser) {
        ...
        await client.set(cacheKey, JSON.stringify(updatedUser), {EX: 100});
    }
    return updatedUser;
};
...
const getUserComments = async (userId) => {
    ...
    await client.set(cacheKey, JSON.stringify(comments), {EX: 500});
    return comments;
};
